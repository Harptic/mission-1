...
public class MainService {
  final String key = "4a474263556b6a7336327974676a71";
  private CommonService commonService = new CommonService();
  private DBConnection dbConnection = new DBConnection();
  private int size = 0;
  public int insetWifi(int start, int end) throws IOException {
    String url = String.format("http://openapi.seoul.go.kr:8088/" + key + "/json/TbPublicWifiInfo/%s/%s/",start , end);
    ...
    if (response.isSuccessful()) {
    	System.out.println("OpenAPI Success");
  ...
  public List<BookmarkResponse> getWifiMarkList() {
